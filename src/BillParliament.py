...
session_years = ["2010-12", "2009-10", "2008-09", "2007-08"] #2007-08 is the earliest for which the scraper works. Year after dash needs to have two digits.
...
for session_year in session_years:
    ...
    billsoutfile = "../output/billsUK_" + session_year + '_' + date + '.csv'
    stagesoutfile = "../output/billsUK_stages_" + session_year + '_' + date + '.csv'
    ...
    for child in rows:
        ...
        if session_present == "TRUE":
            billurl = urlbase + "/" + session_year + + urlstub
        else:
            billurl = urlbase + urlstub           
        try:
        ...
        if billsoup.find("div","next-event"):
            ...
            nextstagename = 'NA'
            nextstagedate = 'NA'
        # get info on MP to allow later matching with MP DB
        if billcat == 'PMB-C':
            # make it searchable - generic replacements
            mpnameuseful = sponsor1.replace('Mr ', '').replace('Dr ', '').replace('Dame ','').replace('Sir ','').replace('Ms ','').replace('Mrs ','').replace('Miss ','')
            # MP-specific replacements
            mpnameuseful = mpnameuseful.replace('Mctaggart', 'Mactaggart').replace('Jeffrey M.','Jeffrey M')
            mpnameuseful = mpnameuseful.replace('Steve Baker','Steven Baker').replace('Nic ', 'Nicholas ').replace('Chris Leslie', 'Christopher Leslie').replace('Steve McCabe','Stephen McCabe')
        ...
        #row2sall = [headerrow2] # this in case I run with the 'dump all stage rows into a list of lists and write it at one go
        for child in srows:
            rowno =+ 1
            ...
            elif stagenumber == numberofstages:
                laststage = "Yes"
                firststage = 'No'
            ...
            row2 = [billurl, session_year, billname, billcat, UKPB_bill, UKPB_Act, RoyalAssent, RoyalAssentDate, stagename, stagedate, stagenumber, numberofstages, firststagename, firststagedatewrite, stagelink]
            #writer2.writerow(row2)
            #row2sall = row2sall.append(row2)
        #for line in row2sall:
            writer2.writerow(row2)
            
        #billduration = (laststagedate - firststagedate)

        row1 = [billurl, session_year, date, billname, typeofbill, billcat, UKPB_bill, UKPB_Act, RoyalAssent, RoyalAssentDate, lastchange, stage, numberofstages, firststagedate, laststagedate, firststagename, laststagename, sponsor1, sponsor2, sponsor1inst, sponsor2inst, parlagent, promotedby, lateststagename, lateststagedate, nextstagename, nextstagedate, billink1, billink2, billinkdate, billfiletitle, latestnews, summary, mpnameuseful, mpid_twfy, mpid_grdn, mpparty, mpmaj, mpentered, mpyearentered, mpconstituency, constid_grdn, mpproblem]
